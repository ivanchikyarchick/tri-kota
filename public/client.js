...
let myUsername = '', myElo = 1000, myCharacter = 'korzhik', mySelectedMode = 1, currentRoom = null, myTeam = null, isSpectator = false; 

const PLAYER_RADIUS = 40, PUCK_RADIUS = 20, WALL_PADDING = 25;

const images = { 
    rink: new Image(), puck: new Image(),
    korzhik: new Image(), karamelka: new Image(), kompot: new Image(),
    goal_korzhik: new Image(), goal_karamelka: new Image(), goal_kompot: new Image()
};

images.rink.src = 'assets/rink.jpg'; images.puck.src = 'assets/puck.png';
images.korzhik.src = 'assets/korzhik.png'; images.karamelka.src = 'assets/karamelka.png'; images.kompot.src = 'assets/kompot.png'; 
images.goal_korzhik.src = 'assets/kor_gol.png'; images.goal_karamelka.src = 'assets/car_gol.png'; images.goal_kompot.src = 'assets/kom_gol.png'; 

const charColors = { korzhik: '0, 100, 255', karamelka: '255, 105, 180', kompot: '0, 200, 0' };
...
socket.on('authResult', (res) => {
    if (res.success) { myUsername = res.username; myElo = res.elo; authDiv.style.display = 'none'; menuDiv.style.display = 'block'; document.getElementById('welcome-text').innerText = `Привет, ${myUsername}!`; if (eloDisplay) eloDisplay.innerText = `🏆 Рейтинг Эло: ${myElo}`; } else { authStatus.innerText = res.msg; authStatus.style.color = 'red'; }
});
...
socket.on('gs', (miniState) => {
    ...
    for (let id in miniState.p) {
        ...
        gameState.players[id].targetX = miniState.p[id].x; gameState.players[id].targetY = miniState.p[id].y; gameState.players[id].targetR = miniState.p[id].r;
        gameState.players[id].ping = miniState.p[id].ping; gameState.players[id].isBot = miniState.p[id].isBot; 
    }

    if (miniState.h === 1) { let snd = hitSounds[Math.floor(Math.random() * hitSounds.length)]; snd.currentTime = 0; snd.play().catch(e => {}); }
});

let eloChangeMsg = ""; 
socket.on('eloUpdated', (data) => {
    myElo = data.elo; if (eloDisplay) eloDisplay.innerText = `🏆 Рейтинг Эло: ${myElo}`;
    if (data.change > 0 && gameState.players[socket.id] && canvas.style.display === 'block') { let p = gameState.players[socket.id]; floatingTexts.push({ x: p.x, y: p.y - 60, text: `+${data.change} ЭЛО!`, life: 90 }); }
...
socket.on('gameOver', (finalScore) => {
    ...
    if (!isSpectator && afkScreen.style.display !== 'flex') {
        if (myTeam === 1 && finalScore.team1 > finalScore.team2) msg = "ВЫ ПОБЕДИЛИ! 🎉"; else if (myTeam === 1 && finalScore.team1 < finalScore.team2) msg = "ВЫ ПРОИГРАЛИ! 😭"; else if (myTeam === 2 && finalScore.team2 > finalScore.team1) msg = "ВЫ ПОБЕДИЛИ! 🎉"; else if (myTeam === 2 && finalScore.team2 < finalScore.team1) msg = "ВЫ ПРОИГРАЛИ! 😭"; else msg = "Ничья!";
    }
...
let chatTimeout;
function toggleChat() { if (chatContainer.style.display === 'none') { chatContainer.style.display = 'flex'; chatToggleBtn.innerText = '👁️'; chatToggleBtn.classList.remove('chat-hidden'); wakeUpChat(); } else { chatContainer.style.display = 'none'; chatToggleBtn.innerText = '🙈'; chatToggleBtn.classList.add('chat-hidden'); } }
function wakeUpChat() { if (chatContainer.style.display === 'none') return; chatContainer.style.opacity = '1'; clearTimeout(chatTimeout); chatTimeout = setTimeout(() => { if (document.activeElement !== document.getElementById('chat-input')) chatContainer.style.opacity = '0.3'; }, 4000); }
document.getElementById('chat-input').addEventListener('focus', () => { chatContainer.style.opacity = '1'; clearTimeout(chatTimeout); }); document.getElementById('chat-input').addEventListener('blur', wakeUpChat);
function sendChat() { const input = document.getElementById('chat-input'); const text = input.value.trim(); if (text && currentRoom) { let senderName = isSpectator ? `[Зритель] ${myUsername}` : myUsername; socket.emit('chatMessage', { roomId: currentRoom, sender: senderName, text: text }); input.value = ''; } }
document.getElementById('chat-input').addEventListener('keypress', function (e) { if (e.key === 'Enter') sendChat(); }); socket.on('chatMessage', (data) => { if (chatContainer.style.display !== 'none') wakeUpChat(); const msgs = document.getElementById('chat-messages'); msgs.innerHTML += `<div><b>${data.sender}:</b> ${data.text}</div>`; msgs.scrollTop = msgs.scrollHeight; });

function getEventPos(e) { const rect = canvas.getBoundingClientRect(), scaleX = canvas.width / rect.width, scaleY = canvas.height / rect.height; let clientX = e.clientX, clientY = e.clientY; if (e.touches && e.touches.length > 0) { clientX = e.touches[0].clientX; clientY = e.touches[0].clientY; } return { x: (clientX - rect.left) * scaleX, y: (clientY - rect.top) * scaleY }; }

function sendInput(pos, dragging) {
    if (isSpectator || !currentRoom) return;
    socket.emit('input', { roomId: currentRoom, dragging: dragging, tx: pos ? pos.x : null, ty: pos ? pos.y : null });
}
...
function safeDrawCircleImage(image, x, y, radius, rotation = 0) {
    ...
    ctx.beginPath(); ctx.arc(0, 0, radius, 0, Math.PI * 2, true); ctx.closePath(); ctx.clip(); ctx.drawImage(image, -radius, -radius, radius * 2, radius * 2); ctx.restore(); 
}
...
function gameLoop(timestamp) {
    ...
    for (let id in gameState.players) {
        ...
        if (p.targetX !== undefined) {
            p.x += (p.targetX - p.x) * 0.4; p.y += (p.targetY - p.y) * 0.4;
            p.rotation += (p.targetR - (p.rotation || 0)) * 0.4;
        }

        let img = images[p.char] || images.korzhik; 
    ...
    let min = Math.floor(gameState.timeLeft / 60), sec = gameState.timeLeft % 60; ctx.fillStyle = gameState.timeLeft <= 10 ? 'red' : 'yellow'; ctx.font = 'bold 24px Arial'; ctx.fillText(`⏱ ${min}:${sec < 10 ? '0' : ''}${sec}`, canvas.width / 2, 85);
    ...
    if (showGoalAnimation) {
        ...
        let flashColor = charColors[goalScorerChar] || '255, 255, 255'; ctx.fillStyle = `rgba(${flashColor}, ${opacity})`; ctx.fillRect(0, 0, canvas.width, canvas.height);
        const goalImg = images[`goal_${goalScorerChar}`] || images.goal_korzhik; if (goalImg.complete) { ctx.globalAlpha = opacity / 0.6; ctx.drawImage(goalImg, canvas.width / 2 - 150, canvas.height / 2 - 150, 300, 300); ctx.globalAlpha = 1.0; }
    }

    for (let i = floatingTexts.length - 1; i >= 0; i--) {
        let ft = floatingTexts[i]; ctx.fillStyle = `rgba(255, 215, 0, ${ft.life / 90})`; ctx.font = 'bold 22px Arial'; ctx.textAlign = 'center';
